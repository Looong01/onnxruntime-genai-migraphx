...
def _parse_args():
    ...
    parser.add_argument("--use_migraphx", action="store_true", help="Whether to use MIGraphX. Default is to not use MIGraphX.")
...
def update(args: argparse.Namespace, env: dict[str, str]):
    ...
    command += [
        ...
        f"-DUSE_MIGRAPHX={'ON' if args.use_migraphx else 'OFF'}",
...
def build(args: argparse.Namespace, env: dict[str, str]):
    ...
    if not args.ort_home:
        _ = util.download_dependencies(args.use_cuda, args.use_rocm, args.use_migraphx, args.use_dml, lib_dir)
    else:
